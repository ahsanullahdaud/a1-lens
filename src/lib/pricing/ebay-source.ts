...
import { assessMatch } from "./match";
import type { MatchQuality, PriceObservation, PriceSource } from "./types";
...
type Candidate = { item: EbayItem; itemPrice: number; shipping: number; landed: number; business: boolean; match: MatchQuality; matchNote?: string };
  ...
  async lookup(product: Product): Promise<PriceObservation[]> {
    ...
    try {
      ...
      for (const item of items) {
        ...
        // The GTIN should pin the variant, but sellers mis-list: check the title for colour/capacity/bundle.
        const { match, note: matchNote } = assessMatch(product.name ?? "", item.title ?? "");
        candidates.push({ item, itemPrice, shipping, landed, business: item.seller?.sellerAccountType === "BUSINESS", match, matchNote });
      }
      ...
      // Exact variants first, then business sellers, then cheapest landed price.
      candidates.sort((a, b) => MATCH_RANK[a.match] - MATCH_RANK[b.match] || Number(b.business) - Number(a.business) || a.landed - b.landed);
      const best = candidates[0];
      const cheaperIndividual = best.business ? candidates.find((c) => !c.business && c.match === best.match && c.landed < best.landed) : undefined;
      const cheaperNear = best.match === "exact" ? candidates.find((c) => c.match !== "exact" && c.landed < best.landed) : undefined;
      ...
      const notes = [
        ...
        cheaperNear ? `a ${cheaperNear.match} match is cheaper at £${cheaperNear.landed.toFixed(2)} (${cheaperNear.matchNote ?? "variant differs"})` : "",
      ].filter(Boolean);
